...
import { ShieldCheck, Lock, Truck, CreditCard, ArrowLeft, Sparkles, CheckCircle2 } from 'lucide-react';

export default function CheckoutPage() {
  const { cart, subtotal, discountAmount, appliedCoupon, totalAmount, clearCart } = useStore();
  const { user } = useAuth();
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10">
      ...
      <form onSubmit={handleOpenPayment} className="grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <div className="lg:col-span-7 space-y-8">
          ...
          <button
            type="submit"
            className="w-full py-4 bg-[#58111A] text-[#FAF6F0] hover:bg-[#D4AF37] hover:text-[#58111A] transition-colors text-xs tracking-[0.25em] uppercase font-semibold flex items-center justify-center gap-2 shadow-xl"
          >
            <Lock className="w-4 h-4" /> PROCEED TO PAY VIA RAZORPAY
          </button>
